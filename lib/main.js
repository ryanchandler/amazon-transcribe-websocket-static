    ...
    if (results.length > 0) {
        if (results[0].Alternatives.length > 0) {
            ...
            if (!results[0].IsPartial) {
                ...
                axios({
                url: 'http://50.82.5.40/interpret?utterance=' + transcript,
                method: 'GET',
                responseType: 'blob', // important
              }).then((response) => {
                //const url = window.URL.createObjectURL(new Blob([response.data]));
                var audio = new Audio(URL.createObjectURL(response.data));
                audio.play();


                audio.addEventListener('ended', (event) => {
                console.log('Audio has stopped either because 1) it was over, ' +
                ...
                //
                // axios.get('http://50.82.5.40/interpret?utterance=' + transcript ).then(resp => {
